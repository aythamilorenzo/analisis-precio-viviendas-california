...
# Instalamos y cargamos las librerías e importamos el dataset desde RStudio
...
```
Este modelo no cumple los supuestos, los residuos no son normales según el método analítico, además, se puede mejorar
...
# Modelo utilizando una T DE STUDENT
...
plot(reg.mult.tstudent)      # Trazas MCMC y densidades
pp_check(reg.mult.tstudent)  # Ver si los residuos ajustan bien
...
Con respecto a las interpretaciones vemos que muchas son similares tras lanzar el summary, excepto porque de este modelo concluimos que la mediana de la antigüedad de las casas no influye en el precio y que al contrario del primer modelo, ahora el hecho de que las casas estén cerca de la bahía si es significativo en el precio.

A pesar de eso, creo que ambos modelos reflejan de una aceptable la relación entre el precio de las casas y las variables explicativas.
